...
module spi_master_core (
    ...
    assign lsbFirst = SPI_CR[8];
    assign dataFrame = SPI_CR[7];
                ...
                IDLE: begin
                    ...
                    if (start) begin
                        ...
                        if (cpha == 1'b0) begin
                            ...
                            if(dataFrame) begin
                                if(lsbFirst) begin
                                    spi_mosi     <= SPI_TX_DATA[0];
                                    tx_shift_reg <= {1'b0, SPI_TX_DATA[15:1]};
                                end else begin
                                    spi_mosi     <= SPI_TX_DATA[15];
                                    tx_shift_reg <= {SPI_TX_DATA[14:0], 1'b0};
                                end
                            end else begin
                                if(lsbFirst) begin
                                    spi_mosi     <= SPI_TX_DATA[0];
                                    tx_shift_reg <= {8'd0, 1'b0, SPI_TX_DATA[7:1]};
                                end else begin 
                                    spi_mosi     <= SPI_TX_DATA[7];
                                    tx_shift_reg <= {8'd0, SPI_TX_DATA[6:0], 1'b0};
                                end
                            end
                        end else begin
                            ...
                            spi_mosi     <= 1'b0;
                            if(dataFrame) begin
                                tx_shift_reg <= SPI_TX_DATA[15:0];
                            end else begin 
                                tx_shift_reg <= {8'd0, SPI_TX_DATA[7:0]};
                            end
                        end
                        state <= DATA;
                    end
                ...
                DATA: begin
                    if (tick) begin
                        ...
                        if ((cpha == 1'b0 && edge_cnt[0] == 1'b0) || (cpha == 1'b1 && edge_cnt[0] == 1'b1)) begin
                            ...
                            if(dataFrame) begin
                                if(lsbFirst) begin
                                    SPI_RX_DATA <= {16'd0, spi_miso, SPI_RX_DATA[15:1]};
                                end else begin
                                    SPI_RX_DATA <= {16'd0, SPI_RX_DATA[14:0], spi_miso};
                                end
                            end else begin
                                if(lsbFirst) begin
                                    SPI_RX_DATA <= {24'd0, spi_miso, SPI_RX_DATA[7:1]};
                                end else begin
                                    SPI_RX_DATA <= {24'd0, SPI_RX_DATA[6:0], spi_miso};
                                end
                            end
                        end else begin
                            // write
                            if(dataFrame) begin
                                ...
                                if(lsbFirst) begin
                                    spi_mosi     <= tx_shift_reg[0];
                                    tx_shift_reg <= {8'd0, 1'b0, tx_shift_reg[7:1]};
                                end else begin
                                    spi_mosi     <= tx_shift_reg[7];
                                    tx_shift_reg <= {8'd0, tx_shift_reg[6:0], 1'b0};
                                end
                            end
                        end
                        if ((dataFrame && edge_cnt == 5'd31) || (!dataFrame && edge_cnt == 5'd15)) begin
                            state <= DONE;
                        end else begin
...
// 0 1 1 100_000_000 / 16 = 6_250_000

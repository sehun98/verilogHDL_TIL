...
// UART_SR [1] : RXD rx_done
// UART_SR [2] : RXF rx_full
// UART_SR [3] : TXE tx_empty
// UART_SR [4] : TXD tx_done
// UART_SR [5] : TXF tx_full
// UART_SR [6] : BUSY rx_tx_busy
// UART_SR [7] : OVR tx_overrun_error
// UART_SR [8] : FE rx_frame_error
...
module axi_uart_core (
    ...
    always_comb begin
        ...
        UART_SR[7] = tx_overrun_error;  // OVR
        UART_SR[8] = rx_frame_error;  // FE
    end
    ...
    assign tx_overrun_error = uart_dr_we && tx_full;

    // Level-sensitive RX interrupt:
    // asserted while unread data remains in the RX FIFO.
    assign irq = rx_en && !rx_empty;
...
module uart_baud_rate (
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            tick <= 1'b0;
            if (!uart_en || (UART_BRR == 32'd0)) begin
                cnt <= 32'd0;
            end else begin
                if (cnt >= (100_000_000 / (UART_BRR * 16)) - 1) begin
                    cnt  <= 32'd0;
                    tick <= 1'b1;
                end else begin
                    cnt <= cnt + 32'b1;

...
module apb_gpio (
    ...
    assign gpio_out = gpio_odr[15:0];
    assign gpio_oe  = gpio_dir;

    genvar i;
    generate
        for (i = 0; i < 16; i++) begin : GPIO_IOBUF
            IOBUF u_iobuf (
                .I (gpio_out[i]),
                .O (gpio_in[i]),
                .T (~gpio_oe[i]),  // T=1이면 High-Z/Input, T=0이면 Output
                .IO(gpio_pin[i])
            );
        end
    endgenerate

    assign gpio_idr = {16'd0, gpio_in};
    ...
    always_ff @(posedge PCLK or negedge PRESETn) begin
        if (!PRESETn) begin
            ...
            case (PADDR[7:0])
                ...
                8'h10: begin
                    gpio_odr[15:0] <= (gpio_odr[15:0] | PWDATA[15:0]) & ~PWDATA[31:16];
                end
                default: ;
            endcase
        end
    ...
    always_comb begin
        ...
        if (apb_read) begin
            case (PADDR[7:0])
                ...
                8'h10:   PRDATA = 32'd0;  // BSRR은 write-only 취급

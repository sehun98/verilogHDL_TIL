...
// UART_SR [1] : RXD rx_data_available
// UART_SR [2] : RXF rx_full
// UART_SR [3] : TXE tx_empty
// UART_SR [4] : TXD tx_complete
// UART_SR [5] : TXF tx_full
// UART_SR [6] : BUSY rx_tx_busy
// UART_SR [7] : TXOVR tx_fifo_overrun
// UART_SR [8] : FE rx_frame_event
// UART_SR [9] : RXOVR rx_fifo_overrun
...
// UART_ICR [1] : reserved (RX interrupt clears when RX FIFO becomes empty)
// UART_ICR [2] : TXOVRC tx_fifo_overrun clear
// UART_ICR [3] : RXOVRC rx_fifo_overrun clear
// UART_ICR [4] : FEC rx_frame_event clear
module axi_uart_core (
    ...
    logic rx_frame_event;
    logic tx_overrun_flag;
    logic rx_overrun_flag;
    logic rx_frame_flag;
    ...
    always_comb begin
        ...
        UART_SR[1] = !rx_empty;  // RXD: receive data available
        UART_SR[2] = rx_full;  // RXF
        UART_SR[3] = tx_empty;  // TXE
        UART_SR[4] = tx_empty && !tx_busy;  // TXD: all transmit complete
        UART_SR[5] = tx_full;  // TXF
        UART_SR[6] = tx_busy | rx_busy;  // BUSY
        UART_SR[7] = tx_overrun_flag;  // TXOVR
        UART_SR[8] = rx_frame_flag;  // FE
        UART_SR[9] = rx_overrun_flag;  // RXOVR
    end
    ...
    assign tx_fifo_push     = uart_dr_we && (!tx_full || tx_fifo_pop);
    assign tx_fifo_pop      = tx_send;

    assign rx_fifo_push     = rx_done && (!rx_full || rx_fifo_pop);
    assign rx_fifo_pop      = uart_dr_re && !rx_empty;

    assign tx_overrun_event = uart_dr_we && tx_full && !tx_fifo_pop;
    assign rx_overrun_event = rx_done && rx_full && !rx_fifo_pop;
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (!uart_en) begin
                tx_irq_pending <= 1'b0;
            end else if (tx_done && tx_empty && !tx_fifo_push) begin
                tx_irq_pending <= 1'b1;
            end else if (UART_ICR[0]) begin
                tx_irq_pending <= 1'b0;
            end
        end
    end
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            tx_overrun_flag     <= 1'b0;
            rx_overrun_flag     <= 1'b0;
            rx_frame_flag <= 1'b0;
        end else begin
            if (tx_overrun_event) begin
                tx_overrun_flag <= 1'b1;
            end else if (UART_ICR[2]) begin
                tx_overrun_flag <= 1'b0;
            end

            if (rx_overrun_event) begin
                rx_overrun_flag <= 1'b1;
            end else if (UART_ICR[3]) begin
                rx_overrun_flag <= 1'b0;
            end

            if (rx_frame_event) begin
                rx_frame_flag <= 1'b1;
            end else if (UART_ICR[4]) begin
                rx_frame_flag <= 1'b0;
            end
        end
    end
    ...
    assign irq = uart_en && ((UART_IER[0] && tx_irq_pending) || (UART_IER[1] && !rx_empty));

    assign UART_DR_RDATA = {24'd0, rx_fifo_pop_data};
    uart_rx u1_uart_rx (
        .clk           (clk),
        .rst_n         (rst_n),
        .tick          (w_tick),
...
module uart_baud_rate (
    ...
    input  logic        rst_n,
    input  logic        uart_en,
    input  logic [31:0] UART_BRR,  // 115200
    output logic        tick
);
    logic [31:0] cnt;
    ...
    always_comb begin
        case (UART_BRR)
            32'd9600:   div_cnt = 32'd650;  // 651 clocks
            32'd19200:  div_cnt = 32'd325;  // 326 clocks
            32'd38400:  div_cnt = 32'd162;  // 163 clocks
            32'd57600:  div_cnt = 32'd108;  // 109 clocks
            32'd115200: div_cnt = 32'd53;   // 54 clocks
            32'd230400: div_cnt = 32'd26;   // 27 clocks
